...
def make_vars(team: dict, agent: dict = None) -> dict:
    ...
    agents_table = "\n".join(
        f"| {a['id']} | {a['name']} | {a['emoji']} | {a['model']} | {a['role']} |"
...
def write_agent_files(team: dict, agent: dict, workspace_root: Path):
    ...
    for filename, content in files.items():
        ...
        print(f"  wrote {agent_dir / filename}")


def write_shared_files(team: dict, workspace_root: Path):
    ...
    for filename, content in shared.items():
        ...
        print(f"  wrote {workspace_root / filename}")
    ...
    for agent in team["agents"]:
        ...
        (workspace_root / "comms" / "inboxes" / f"{aid}.md").write_text(
            f"# {agent['name']} Inbox\n\n_No messages._\n", encoding="utf-8"
        )
        (workspace_root / "comms" / "outboxes" / f"{aid}.md").write_text(
            f"# {agent['name']} Outbox\n\n_No messages._\n", encoding="utf-8"
        )
    (workspace_root / "comms" / "broadcast.md").write_text(
        f"# {team['team_name']} Broadcast\n\n_No broadcasts._\n", encoding="utf-8"
    )
...
def main():
    ...
    if args.agent:
        ...
        write_agent_files(team, agent, workspace_root)
    ...
    if args.shared:
        ...
        write_shared_files(team, workspace_root)
