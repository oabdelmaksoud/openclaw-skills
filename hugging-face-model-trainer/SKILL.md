        ...
        "install": [
          {
            ...
            "bins": ["hf"],
            "label": "Install HuggingFace CLI (pip)"
          }
        ]
      }
  }
...
UV scripts use PEP 723 inline dependencies for clean, self-contained training. **This is the primary approach for OpenClaw.**
...
**When to use:** Default choice for all training tasks in OpenClaw, custom training logic, any scenario requiring `hf_jobs()`
...
**When to use:** User working in terminal directly (not OpenClaw context), quick local experimentation
**Repository:** https://github.com/huggingface/trl-jobs

⚠️ **In OpenClaw context, prefer using `hf_jobs()` MCP tool (Approach 1) when available.**
...
7. **Use UV scripts (Approach 1)** - Default to `hf_jobs("uv", {...})` with inline scripts; TRL maintained scripts for standard training; avoid bash `trl-jobs` commands in OpenClaw
