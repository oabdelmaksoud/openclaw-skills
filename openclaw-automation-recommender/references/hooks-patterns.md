...
Hooks automatically run commands in response to OpenClaw events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when OpenClaw sends notifications. Use matchers to filter by notification type.
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Claude is waiting\" with title \"OpenClaw\"'"
          }
        ]
      }
    ]
