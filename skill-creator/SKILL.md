...
- Create a few test prompts and run openclaw-with-access-to-the-skill on them
...
- **metadata**: OpenClaw metadata block with emoji
...
skill-name/
├── SKILL.md (required)
│   ├── YAML frontmatter (name, description, metadata required)
...
Skills are stored in `~/.openclaw/skills/` and are available to all 16 agents in the OpenClaw roster.
...
3. **Explain the why.** Try hard to explain the **why** behind everything you're asking the agent to do. Today's LLMs are *smart*. They have good theory of mind and when given a good harness can go beyond rote instructions and really make things happen. Even if the feedback from the user is terse or frustrated, try to actually understand the task and why the user is writing what they wrote, and what they actually wrote, and then transmit this understanding into the instructions. If you find yourself writing ALWAYS or NEVER in all caps, or using super rigid structures, that's a yellow flag — if possible, reframe and explain the reasoning so that the agent understands why the thing you're asking for is important. That's a more humane, powerful, and effective approach.
...
The description field in SKILL.md frontmatter is the primary mechanism that determines whether an OpenClaw agent invokes a skill. After creating or improving a skill, offer to optimize the description for better triggering accuracy.
...
After packaging, direct the user to the resulting `.skill` file path so they can install it or share it with other OpenClaw users.
...
The same principles from the main improvement section apply. Additionally, for OpenClaw skills:

- Consider which agents will most commonly use this skill — tailor language accordingly
- Skills in `~/.openclaw/skills/` are available to ALL 16 agents, so keep them general unless the skill should only be used by specific agents
- If a skill is agent-specific, mention that in the description (e.g., "This skill is primarily for Forge (implementation agent)")
...
- Run openclaw-with-access-to-the-skill on test prompts
